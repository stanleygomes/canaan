...
async def scheduled_scrape() -> None:
    try:
        ...
        logger.info("Execução agendada iniciada: %s", run.run_id)
    except ScrapeAlreadyRunning:
        logger.warning("Execução agendada ignorada: já existe uma execução em andamento")
    except DatabaseUnavailable as error:
        logger.error("Execução agendada não iniciada: %s", error)
...
async def main() -> None:
    ...
    scheduler.start()
    logger.info("Cron ativo: '%s' (%s)", settings.scrape_cron, settings.scrape_timezone)
    try:
