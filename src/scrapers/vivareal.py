...
class VivaRealScraper:
    ...
    async def extract_listing_cards(self, page: Page) -> List[Dict[str, Any]]:
        """Extrai todos os cards da listagem da página de busca do Viva Real."""
        ...
        for rc in raw_cards:
            ...
            for i, line in enumerate(lines):
                if "Apartamento para comprar com" in line or "Imóvel para" in line:
                    ...
                    if i + 1 < len(lines):
                        locality = lines[i + 1]
                    if i + 2 < len(lines) and ("Rua " in lines[i + 2] or "Avenida " in lines[i + 2] or "Alameda " in lines[i + 2]):
                        street = lines[i + 2]
                elif line.startswith("R$ ") and price is None:
                    price = clean_currency(line)
                elif "Cond." in line or "IPTU" in line:
                    # Ex: 'Cond. R$ 445 • IPTU R$ 422'
            ...
            # Anunciante (geralmente uma das primeiras linhas não numéricas)
            for l in lines[:3]:
    ...
    async def run(
        self,
        search_url: str = "https://www.vivareal.com.br/venda/parana/curitiba/apartamento_residencial/",
        max_pages: int = 1,
        ...
        async with async_playwright() as p:
            ...
            for pg in range(1, max_pages + 1):
                ...
                logger.info("🌐 Acessando Viva Real página {}: {}", pg, url)

                try:
                    await navigate_with_rate_limit(page, url, "vivareal")
                    await page.wait_for_timeout(2500)
                except Exception as e:
                    logger.opt(exception=e).error("❌ Erro ao carregar listagem Viva Real: {}", url)
                    continue

                cards = await self.extract_listing_cards(page)
                logger.info("📦 {} imóveis extraídos da página {} do Viva Real", len(cards), pg)
        ...
        logger.info("✅ Scraping Viva Real finalizado: {} imóveis salvos em '{}'", len(all_properties), output_file)
        return all_properties


if __name__ == "__main__":
    scraper = VivaRealScraper(headless=True)
    asyncio.run(
        scraper.run(
            search_url="https://www.vivareal.com.br/venda/parana/curitiba/apartamento_residencial/",
            max_pages=1,
            max_properties=3,
            output_file="vivareal_imoveis.json",
        )
    )
